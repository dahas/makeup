...
class App extends Module
{
    ...
    public function render($modName = "")
    {
        ...
        $SubMetaHttpEquiv = $this->getTemplate()->getSubpart('%CONF_HTTP_EQUIV%');
        $subpart['%CONF_HTTP_EQUIV%'] = '';
        if (Config::get('meta_http_equiv')) {
            foreach (Config::get('meta_http_equiv') as $equiv => $content) {
                $equivMarker['%HTTP_EQUIV%'] = $equiv;
                $equivMarker['%CONTENT%'] = $content;
                $subpart['%CONF_HTTP_EQUIV%'] .= $SubMetaHttpEquiv->parse($equivMarker);
            }
        }
        ...
        if (Config::get('metatags')) {
            foreach (Config::get('metatags') as $meta => $content) {
